...
const LOCALES: Record<string, EditorMessages> = {
	ko,
	en,
	ja,
	es,
...
function messagesForCode(code: string): EditorMessages {
 ...
 */
export function createTranslator(locale?: EditorLocaleInput): EditorTranslator {
	const overrides: Partial<EditorMessages> | null =
		locale && typeof locale === 'object' ? locale : null;
	const messages: EditorMessages =
		typeof locale === 'string' ? messagesForCode(locale) : ko;
	return (key, params) => {
...
export const defaultTranslator: EditorTranslator = createTranslator();
 ...
 */
export const EditorI18n = Extension.create<EditorI18nOptions, EditorI18nStorage>({
	name: 'editorI18n',
 ...
 */
export function getEditorTranslator(
	editor: { storage?: unknown } | null | undefined
): EditorTranslator {
	const storage = editor?.storage as
		| { editorI18n?: { t?: EditorTranslator } }
		| undefined;
