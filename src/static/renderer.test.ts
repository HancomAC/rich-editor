...
const pdfFixture = vi.hoisted(() => ({ sources: [] as string[], pages: [] as number[] }));
...
describe("rich editor static rendering", () => {
  ...
  it("mounts an existing NodeView without creating a Tiptap Editor", () => {
    ...
    const session = renderer.mount(
      target,
      '<div data-file-src="/files/report.pdf" data-file-name="report.pdf" data-file-size="2048"></div>',
    );
  ...
  it("hydrates card content into the prepared content hole", () => {
    ...
    renderer.mount(
      target,
      '<div data-type="card" data-card-title="Notice"><p>body</p></div>',
    );
