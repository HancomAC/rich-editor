...
describe("static package entry compatibility", () => {
  it("bundles the legacy public entry for SSR without Svelte or the editor runtime", async () => {
    ...
    const entry = manifest.exports["./static"].default as string;
    ...
    if (entryChunk?.type !== "chunk") throw new Error("Missing static entry chunk");
  ...
  it("exposes the Svelte component through a separate public entry", async () => {
    ...
    const sourceEntry = (componentEntry.default as string)
      .replace(/^\.\/dist\//, "./src/")
      .replace(/\.js$/, ".ts");
