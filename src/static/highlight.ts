...
/**
 * 하이라이터가 돌려주는 hast 트리에서 **실제로 읽는 것만** 추린 모양. `value`·`className`을
 * `unknown`으로 두는 건 hast의 실제 타입이 더 넓어서다 — 좁게 적으면 lowlight 반환값이
 * 대입되지 않아 캐스트가 필요해진다.
 */
type HighlightNode = {
  type?: string;
...
function countHighlightTokens(tree: HighlightNode): number {
  let total = 0;
  const walk = (node: HighlightNode): void => {
    if (node.type === "element") total += 1;
 ...
 */
function autoHighlight(value: string): HighlightNode {
  const auto = lowlight.highlightAuto(value, { subset: AUTO_DETECT_SUBSET });
  if ((auto.data as { language?: string } | undefined)?.language) return auto;
  let best: { count: number; tree: HighlightNode } | null = null;
  for (const language of AUTO_DETECT_SUBSET) {
    ...
    if (!best || count > best.count) best = { count, tree };
  }
 ...
 */
export function highlightCodeToHtml(language: string, text: string): string {
  /* `registered`를 먼저 본다 — 목록을 만들지 않는 조회라 블록마다 불려도 싸다. */
  const known =
    !!language && (lowlight.registered(language) || lowlight.listLanguages().includes(language));
 ...
 */
function serializeHighlight(tree: HighlightNode): string {
  /* 확장의 `getHighlightNodes`와 같게 — 옛 hljs 결과는 `value`, lowlight는 `children`이다. */
  const roots: HighlightNode[] = Array.isArray(tree.value)
    ? (tree.value as HighlightNode[])
    : (tree.children ?? []);
  let out = "";
  const walk = (node: HighlightNode, inherited: string[]): void => {
