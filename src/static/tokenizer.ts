...
export function decodeEntities(value: string): string {
  ...
  return value.replace(
    ...
    (match, decimal: string | undefined, hex: string | undefined, name: string | undefined) => {
      if (decimal) return codePointToString(parseInt(decimal, 10), match);
      if (hex) return codePointToString(parseInt(hex, 16), match);
...
function parseAttributes(region: string): StaticAttr[] {
...
export function parseHtml(html: string): StaticRoot {
  const root: StaticRoot = { children: [] };
  const stack: StaticElement[] = [];
  const lower = html.toLowerCase();
  const top = (): StaticNode[] => (stack.length ? stack[stack.length - 1].children : root.children);
  const pushText = (raw: string) => {
    if (raw) top().push({ type: "text", raw });
  };
  ...
  while (index < html.length) {
    ...
    if (next === "/") {
      ...
      while (stack.length > open + 1) stack.pop();
    ...
    if (next && /[a-zA-Z]/.test(next)) {
      ...
      if (selfClosing) region = region.replace(/\/\s*$/, "");
      ...
      if (RAW_TEXT_ELEMENTS.has(name)) {
        ...
        if (close === -1) {
          if (index < html.length) element.children.push({ type: "raw", raw: html.slice(index) });
          index = html.length;
        } else {
          if (close > index) element.children.push({ type: "raw", raw: html.slice(index, close) });
...
export function serializeHtml(root: StaticRoot): string {
  let out = "";
...
function buildStartTag(element: StaticElement): string {
  const attrs = element.attrs
    .map((attr) =>
      attr.value === null ? ` ${attr.rawName}` : ` ${attr.rawName}="${escapeHtml(attr.value)}"`,
    )
    .join("");
...
export function getAttribute(element: StaticElement, name: string): string | null {
  const found = element.attrs.find((attr) => attr.name === name);
  if (!found) return null;
  return found.value ?? "";
}

export function setAttribute(element: StaticElement, name: string, value: string): void {
  const found = element.attrs.find((attr) => attr.name === name);
  if (found) found.value = value;
  else element.attrs.push({ name, rawName: name, value });
  element.dirty = true;
}

export function firstElementChild(element: StaticElement): StaticElement | null {
...
export function textContent(element: StaticElement): string {
  let out = "";
  const walk = (node: StaticNode): void => {
    if (node.type === "text") out += decodeEntities(node.raw);
