 ...
 */
export const FLOATING_HELPER_ON_CLASS = "hce-floating-helper-on";
 ...
 */
export function isEmptyRootParagraph(state: EditorState): boolean {
 ...
 */
export function floatingHelperVisible(
  editor: Editor,
  opts: { focused: boolean; suppressed: boolean },
): boolean {
  if (!editor.isEditable) return false;
  if (!opts.focused || opts.suppressed) return false;
  return isEmptyRootParagraph(editor.state);
}
 ...
 * (입력 규칙 제외) 그 관례를 따른다.
 */
export function helperInsertMath(editor: Editor): boolean {
  return editor.chain().focus().promptMathDisplay().run();
}

/** 코드 버튼. 슬래시 메뉴의 `코드` 항목과 같은 커맨드다. */
export function helperInsertCodeBlock(editor: Editor): boolean {
