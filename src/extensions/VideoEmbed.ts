...
declare module "@tiptap/core" {
	interface Commands<ReturnType> {
		videoEmbed: {
			setVideoEmbed: (attrs: { src: string; width?: string }) => ReturnType;
		};
	}
}
 ...
 * 없다** — 읽는 쪽에서 거른다.
 */
function safeVideoSrc(raw: unknown): string | null {
...
export function toEmbedUrl(raw: string): string {
	...
	if (host === "vimeo.com" || host === "player.vimeo.com") {
		if (host === "player.vimeo.com") return trimmed; // 이미 플레이어 주소
...
export const VideoEmbed = Node.create<VideoEmbedOptions>({
	...
	renderHTML({ HTMLAttributes }) {
		...
		if (height != null) attrs["data-video-height"] = String(height);
		const ratio = normalizeMediaRatio(HTMLAttributes.ratio);
		if (height == null && ratio) attrs["data-video-ratio"] = ratio;
