...
describe('VideoBlock extension', () => {
	...
	it('parses <div data-video-src="url">', () => {
		createEditor(
			'<div data-video-src="https://example.com/video.mp4" data-video-name="demo"></div>'
		);
		const doc = editor.getJSON();
		const videoNode = doc.content?.find(
			(n: Record<string, unknown>) => n.type === 'videoBlock'
		);
	...
	it('defaults name to empty string', () => {
		createEditor(
			'<div data-video-src="https://example.com/video.mp4"></div>'
		);
		const doc = editor.getJSON();
		const videoNode = doc.content?.find(
			(n: Record<string, unknown>) => n.type === 'videoBlock'
