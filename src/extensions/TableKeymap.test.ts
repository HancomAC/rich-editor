...
describe("TableKeymap", () => {
	...
	function posOf(text: string): number {
		...
		editor.state.doc.descendants((node, p) => {
			...
			if (node.isText && node.text?.includes(text)) pos = p;
			return pos === -1;
		});
		if (pos === -1) throw new Error(`"${text}" not found`);
		return pos;
	}
	...
	function cellPositions(): number[] {
		const cells: number[] = [];
		editor.state.doc.descendants((node, p) => {
			if (node.type.name === "tableCell" || node.type.name === "tableHeader") cells.push(p);
			return true;
		});
		return cells;
	}
	...
	const EMPTY_TABLE =
		"<table><tbody><tr><td><p></p></td><td><p></p></td></tr></tbody></table><p>after</p>";
