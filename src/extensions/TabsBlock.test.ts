...
/** NodeView 가 만든 탭바. */
const bar = (e: Editor) => e.view.dom.querySelector(".hce-tabs-bar") as HTMLElement;
const chips = (e: Editor) => Array.from(bar(e).querySelectorAll(".hce-tab-chip"));
const labels = (e: Editor) =>
	chips(e).map((chip) => chip.querySelector(".hce-tab-chip-label")?.textContent ?? "");
const click = (el: Element) =>
	el.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));

/*
 * ⚠️ `getJSON()` 의 반환 타입은 스키마에서 유도돼 자식이 `노드 | 텍스트` 유니온이다 —
 *    `.content`·`.attrs` 를 바로 파고들면 텍스트 쪽에 없다고 타입 에러가 난다.
 *    검사에 필요한 모양만 적어 한 번 좁힌다(`Columns.test.ts` 가 `Record<string, unknown>`
 *    으로 피해 가는 것과 같은 이유, 여기는 더 깊이 들어가서 이름을 붙였다).
 */
type Json = { type?: string; attrs?: Record<string, unknown>; content?: Json[] };
const tabsNode = (e: Editor): Json | undefined =>
	(e.getJSON() as Json).content?.find((n) => n.type === "tabs");

/** 문서에 든 탭들의 제목(속성 그대로 — 기본값 대체 없음). */
const titles = (e: Editor) =>
	(tabsNode(e)?.content ?? []).map((t) => String(t.attrs?.title ?? ""));

/** 탭마다 `[제목, 본문 글자]`. 어느 탭에 글이 들어갔는지 한눈에 보려고 쓴다. */
const titlesAndText = (e: Editor): [string, string][] => {
...
const CHIP_W = 90;
const CHIP_GAP = 100;
const rect = (left: number, right: number, top = 0, bottom = 30) =>
	({
		left,
		right,
		top,
...
/** i 번째 칩의 가로 중앙 좌표. */
const chipCenter = (i: number) => i * CHIP_GAP + CHIP_W / 2;

const pointer = (el: Element, type: string, x: number, y = 15, pointerId = 1) =>
	el.dispatchEvent(
 ...
 */
const drag = (e: Editor, from: number, x: number, y = 15, commit = true) => {
	stubGeometry(e);
...
const activeIndexes = (e: Editor) => {
	const has = (list: Element[], cls: string) =>
		list.findIndex((el) => el.classList.contains(cls));
...
describe("TabsBlock — 탭바(NodeView)", () => {
	...
	it("⚠️ 마지막 한 개는 지울 수 없다 — 버튼은 자리만 지킨다", async () => {
		 ...
		 */
		const close = chips(e)[0].querySelector(".hce-tab-chip-close") as HTMLButtonElement;
	...
	it("칩을 두 번 누르면 이름을 고칠 수 있다", async () => {
		...
		const input = bar(e).querySelector(".hce-tab-title-input") as HTMLInputElement;
	...
	it("⚠️ 회귀 B: 커서가 옮겨지는 순간 두 탭이 **모두** 보인다", async () => {
		...
		const panels = e.view.dom.querySelector(".hce-tabs-panels")!;
		let atMove: { 대상: boolean; 이전: boolean } | null = null;
		const watch = ({ transaction }: { transaction: { selectionSet: boolean } }) => {
			if (!transaction.selectionSet || atMove !== null) return;
			atMove = {
