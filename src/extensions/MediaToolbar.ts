...
import { getEditorTranslator, type EditorMessageKey, type EditorTranslator } from "../i18n";
 ...
 */
const DEFAULT_TYPES: Record<string, MediaToolbarTypeConfig> = {
	image: { align: true },
 ...
 */
export function applyMediaToolbarAction(
	attrs: Record<string, unknown>,
	action: string,
	value: string,
	config: MediaToolbarTypeConfig
	...
	if (action === "width" && config.widthPresets) {
		if (!(WIDTH_PRESETS as readonly string[]).includes(value)) return null;
	...
	if (config.ratio) {
		...
		groups.push(
			RATIO_PRESETS.map((preset) =>
				chip(pos, "ratio", preset, preset, currentRatio === preset)
			)
		);
	}
	...
	if (config.align) {
		...
		groups.push(
			ALIGN_PRESETS.map((preset) =>
				chip(pos, "align", preset.value, t(preset.labelKey), currentAlign === preset.value)
			)
		);
	}
	...
	if (config.widthPresets) {
		groups.push(
			WIDTH_PRESETS.map((preset) =>
				chip(pos, "width", preset, preset, node.attrs.width === preset)
			)
		);
	}

	groups.forEach((buttons, index) => {
		if (index > 0) toolbar.appendChild(separator());
...
function toolbarKey(pos: number, node: ProseMirrorNode): string {
	return `media-toolbar-${pos}-${node.type.name}-${String(node.attrs.width ?? "")}-${String(
		node.attrs.height ?? ""
	)}-${String(node.attrs.ratio ?? "")}-${String(node.attrs.align ?? "")}`;
}

export const MediaResizeToolbar = Extension.create<MediaResizeToolbarOptions>({
	...
	addProseMirrorPlugins() {
		const getConfig = (node: ProseMirrorNode): MediaToolbarTypeConfig | null =>
			this.options.types[node.type.name] ?? null;
				...
				props: {
					decorations: (state) => {
						...
						return DecorationSet.create(state.doc, [
							Decoration.widget(
								pos + node.nodeSize,
								() => buildToolbar(pos, node, config, getEditorTranslator(this.editor)),
								{ side: 1, key: toolbarKey(pos, node) }
							)
						]);
					},
					handleDOMEvents: {
						...
						mousedown: (view, event) => {
							...
							const button =
								event.target.closest<HTMLElement>(".hce-media-toolbar-chip");
							...
							const nextAttrs = applyMediaToolbarAction(
								node.attrs,
								button.dataset.action || "",
								button.dataset.value || "",
								config
							);
							if (!nextAttrs) return true;
