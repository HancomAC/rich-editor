...
describe('PdfBlock extension', () => {
	...
	describe('node view — 판 위 조작부', () => {
		...
		async function mountLoaded(html = PDF_HTML, editable = true) {
			...
			for (let i = 0; i < 50; i++) {
				const b = m.overlay.querySelector('.pdf-preset') as HTMLButtonElement;
		...
		it('keeps the presets disabled until the page size is known', async () => {
			const { overlay } = mount();
			expect(
				[...overlay.querySelectorAll('.pdf-preset')].every(
			...
			await mountLoaded();
			expect(
				[...editor.view.dom.querySelectorAll('.pdf-preset')].every(
					(b) => !(b as HTMLButtonElement).disabled
				)
			).toBe(true);
		});
		...
		it('stores an absolute width computed from the page size', async () => {
			...
			const click = (label: string) =>
				(
					[...overlay.querySelectorAll('.pdf-preset')].find(
		...
		it('redraws the canvas on a width change without waiting for an observer', async () => {
			const { overlay } = await mountLoaded();
			const frame = editor.view.dom.querySelector('.pdf-frame') as HTMLElement;
		...
		it('rings the selected media blocks in the brand colour', () => {
			...
			const rule =
				css.match(
					/\.tiptap\[contenteditable='true'\] img\.ProseMirror-selectednode[^{]*\{[^}]*\}/
				)?.[0] ?? '';
			...
			const legacy =
				css.match(/\.ProseMirror-selectednode:not\(img\)[^{]*\{/)?.[0] ?? '';
		...
		it('leaves a legacy percentage alone', async () => {
			...
			expect(editor.getHTML()).toContain('data-pdf-width="50%"');
			expect(
				(editor.view.dom.querySelector('[data-type="pdfBlock"]') as HTMLElement)
					.style.width
			).toBe('50%');
		});
		...
		it('does not let clicks select the control text', () => {
			...
			const inputRule =
				css.match(/\.pdf-overlay \.pdf-page-input\s*\{[^}]*\}/)?.[0] ?? '';
