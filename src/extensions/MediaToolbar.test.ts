...
import type { MidibusRawAttrs } from "./TiptapMidibus";
...
describe("크기·정렬 속성 왕복", () => {
	...
	function createEditor(extensions: AnyExtension[], content: string) {
		editor = new Editor({
	...
	function findNode(typeName: string): { pos: number; node: ProseMirrorNode } | null {
		let found: { pos: number; node: ProseMirrorNode } | null = null;
		editor.state.doc.descendants((node, pos) => {
			if (found) return false;
			if (node.type.name === typeName) found = { pos, node };
			return found === null;
		});
		return found;
	}
	...
	it("VideoEmbed — 높이·정렬이 data-* 로 돌아온다", () => {
		createEditor(
			[VideoEmbed],
			'<div data-video-src="https://www.youtube.com/embed/abc" data-video-width="480px" data-video-height="360" data-video-align="center"></div>'
		);
	...
	it("VideoEmbed — 비율은 높이가 없을 때만 나간다(배타)", () => {
		createEditor(
			[VideoEmbed],
			'<div data-video-src="https://www.youtube.com/embed/abc" data-video-ratio="16:9"></div>'
		);
		expect(editor.getHTML()).toContain('data-video-ratio="16:9"');

		editor.commands.setContent(
			'<div data-video-src="https://www.youtube.com/embed/abc" data-video-height="360" data-video-ratio="16:9"></div>'
		);
		const html = editor.getHTML();
		expect(html).toContain('data-video-height="360"');
	...
	it("MbusVideo — 같은 규칙, 이름만 data-mbus-*", () => {
		createEditor(
			[MbusVideo],
			'<div data-mbus-src="https://play.mbus.tv/v1/hls/x" data-mbus-height="500" data-mbus-align="right"></div>'
		);
		const html = editor.getHTML();
	...
	it("이미지 — 정렬은 style 이 아니라 data-align 속성으로", () => {
		createEditor(
			...
			'<img src="/a.png" width="480" data-align="center">'
		);
	...
	it("이미지 — 모르는 정렬 값은 버린다", () => {
		createEditor(
			[ResizableImage.configure({ inline: false })],
			'<img src="/a.png" data-align="middle">'
		);
		expect(editor.getHTML()).not.toContain("data-align");
	});

	it("살균기가 새 속성을 살려 보낸다 — 허용 목록 정합", () => {
		const media = sanitizeHtml(
			'<div data-video-src="https://www.youtube.com/embed/a" data-video-height="360" data-video-ratio="16:9" data-video-align="center" data-mbus-height="500" data-mbus-ratio="4:3" data-mbus-align="left"></div>'
		);
		for (const attr of [
		...
		expect(sanitizeHtml('<img src="/a.png" width="480" data-align="center">')).toContain(
			'data-align="center"'
		);
	});
	...
	it("TiptapMidibus — 높이 갱신이 rawAttrs 와 함께 가고 나머지는 바이트 보존", () => {
		const source =
			'<tiptap-midibus id="abc" start="5" uuid="u-1" width="100%" height="600" data-bubble-menu="false"></tiptap-midibus>';
		createEditor([TiptapMidibus], source);
		expect(editor.getHTML()).toContain(
			'id="abc" start="5" uuid="u-1" width="100%" height="600" data-bubble-menu="false"'
		);

		const found = findNode("tiptapMidibus")!;
		const raw = found.node.attrs.rawAttrs as MidibusRawAttrs;
		// 높이 드래그 커밋과 같은 트랜잭션(`TiptapMidibus` 노드뷰의 `buildAttrs`)
		editor.view.dispatch(
			editor.view.state.tr.setNodeMarkup(found.pos, undefined, {
				...found.node.attrs,
				height: "420",
		...
		expect(editor.getHTML()).toContain(
			'id="abc" start="5" uuid="u-1" width="100%" height="420" data-bubble-menu="false"'
		);
	});
});
...
describe("MediaResizeToolbar 데코레이션", () => {
	...
	const CONTENT =
		'<p>글</p><div data-video-src="https://player.vimeo.com/video/1"></div>';
	...
	function videoPos(): number {
		...
		editor.state.doc.descendants((node, pos) => {
			...
			if (node.type.name === "videoEmbed") at = pos;
			return at < 0;
		});
		return at;
	}
	...
	it("칩 mousedown 이 속성을 바꾸고 저장본까지 간다", () => {
		createEditor();
		editor.commands.setNodeSelection(videoPos());
		const chip = editor.view.dom.querySelector<HTMLElement>(
			'.hce-media-toolbar-chip[data-action="align"][data-value="center"]'
		);
		...
		const pressed = editor.view.dom.querySelector(
			'.hce-media-toolbar-chip[data-action="align"][data-value="center"]'
		);
	...
	it("비율 칩은 높이를 풀면서 들어간다", () => {
		...
		const chip = editor.view.dom.querySelector<HTMLElement>(
			'.hce-media-toolbar-chip[data-action="ratio"][data-value="4:3"]'
		);
		chip!.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, cancelable: true }));
