...
function tryCreateNodeSelection(doc: ProseMirrorNode, pos: number): NodeSelection | null {
			...
			height: {
				...
				parseHTML: (element) => {
					const value = Number.parseInt(
						element.getAttribute("data-upload-height") || "",
						10
					);
				...
				renderHTML: (attributes) => {
					const raw = Number.parseFloat(
						String(attributes.height ?? DEFAULT_HEIGHT.block)
					);
	...
	addNodeView() {
		return ({ node }) => {
			...
			const apply = (current: ProseMirrorNode) => {
				...
				const kind =
					typeof rawKind === "string" && rawKind.trim().length ? rawKind : "block";
			...
			return {
				dom,
				update: (updated) => {
					if (updated.type.name !== UPLOAD_SKELETON_NODE) return false;
					apply(updated);
					return true;
				}
			};
