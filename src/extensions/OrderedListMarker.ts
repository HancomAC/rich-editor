 ...
 */
const canJoinSequentialList = (node: ProseMirrorNode, type: string, markerOrdinal: number): boolean => {
	const nodeType = (node.attrs.type as string | null) || '1';
...
export const OrderedListMarker = OrderedList.extend({
	renderHTML({ HTMLAttributes }) {
		const { start, type, ...rest } = HTMLAttributes as Record<string, unknown>;
		const attrs: Record<string, unknown> = { ...rest };
		// 기본 확장과 같은 규칙 — start 1 은 안 적는다(적으나 안 적으나 같은 목록이다).
		if (start !== undefined && start !== null && start !== 1) attrs.start = start;
		if (type) {
			attrs.type = type;
		...
		return [
			...
			wrappingInputRule({
				...
				joinPredicate: (match, node) =>
					canJoinSequentialList(node, 'A', match[1].charCodeAt(0) - 64)
			}),
			wrappingInputRule({
				find: /^([a-z])\.\s$/,
				type: this.type,
				...
				joinPredicate: (match, node) =>
					canJoinSequentialList(node, 'a', match[1].charCodeAt(0) - 96)
			}),
			wrappingInputRule({
				...
				joinPredicate: (match, node) =>
					canJoinSequentialList(node, 'kors', 1 + KOREAN_CONSONANTS.indexOf(match[1]))
			}),
			wrappingInputRule({
				...
				joinPredicate: (match, node) =>
					canJoinSequentialList(node, 'korc', 1 + KOREAN_SYLLABLES.indexOf(match[1]))
			})
		];
	}
});
