...
describe("CardBlock", () => {
	it("새 스키마(div[data-type=card])를 카드로 읽는다", () => {
		const e = make(
			'<div data-type="card" data-card-title="제목" data-card-background="red" data-card-height="240"><p>본문</p></div>'
		);
	...
	it("내용을 가진 컨테이너다 — 자식 블록이 보존된다", () => {
		const e = make(
			'<div data-type="card"><p>첫째</p><p>둘째</p></div>'
		);
		const node = e.getJSON().content?.[0];
...
describe("transformLegacyHtml — tiptap-card", () => {
	...
	it("중첩 블록이 있는 본문을 통째로 보존한다", () => {
		const out = transformLegacyHtml(
			'<tiptap-card title="T"><p>하나</p><ul><li>둘</li></ul></tiptap-card>'
		);
	...
	it("카드가 둘 이상이어도 각각 바꾼다", () => {
		const out = transformLegacyHtml(
			'<tiptap-card title="A"><p>1</p></tiptap-card><tiptap-card title="B"><p>2</p></tiptap-card>'
		);
