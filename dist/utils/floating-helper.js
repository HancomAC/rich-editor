 ...
 */
export function isEmptyRootParagraph(state) {
 ...
 */
export function floatingHelperVisible(editor, opts) {
    if (!editor.isEditable)
        return false;
    if (!opts.focused || opts.suppressed)
        return false;
 ...
 * (입력 규칙 제외) 그 관례를 따른다.
 */
export function helperInsertMath(editor) {
    return editor.chain().focus().promptMathDisplay().run();
}
/** 코드 버튼. 슬래시 메뉴의 `코드` 항목과 같은 커맨드다. */
export function helperInsertCodeBlock(editor) {
