...
describe("createStaticSanitizePolicy", () => {
    it("preserves editor data attributes and safe formatting", () => {
        const html = sanitizeTiptapHTML('<div data-file-src="/files/a.pdf" data-file-name="a.pdf"></div><p style="color: red; position: fixed">safe</p>', [], createStaticSanitizePolicy());
        expect(html).toContain('data-file-src="/files/a.pdf"');
        expect(html).toContain("color:red");
        expect(html).not.toContain("position");
    });
    it("does not enable active content unless the caller opts in", () => {
        const html = sanitizeTiptapHTML('<iframe src="https://evil.example/frame"></iframe>', [], createStaticSanitizePolicy());
        expect(html).toBe("");
    });
    it("merges caller attributes with the package defaults", () => {
        const html = sanitizeTiptapHTML('<div data-file-name="a.pdf" data-custom="yes"></div>', [], createStaticSanitizePolicy({ allowedAttributes: { div: ["data-custom"] } }));
    ...
    it("preserves read-only task list state", () => {
        const html = sanitizeTiptapHTML('<ul data-type="taskList"><li data-type="taskItem" data-checked="true"><label><input type="checkbox" checked="checked"><span></span></label><div><p>done</p></div></li></ul>', [], createStaticSanitizePolicy());
    ...
    it("composes a caller input transform before enforcing read-only checkboxes", () => {
        const html = sanitizeTiptapHTML('<input type="checkbox" data-custom="yes">', [], createStaticSanitizePolicy({
