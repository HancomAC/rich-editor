 ...
 * 쓰던 방어를 그대로 옮겼다.
 */
function safeRegExp(withLookbehind, fallback) {
    try {
 ...
 */
function getPrompt(editor) {
    const storage = editor.storage;
    const inline = storage[INLINE_NAME];
    const display = storage[DISPLAY_NAME];
    return inline?.promptMath ?? display?.promptMath ?? null;
}
async function askMath(editor, current, displayMode) {
...
async function promptInsert(editor, displayMode) {
    ...
    if (displayMode)
        editor.chain().focus().insertMathDisplay(latex).run();
    else
        editor.chain().focus().insertMathInline(latex).run();
}
/** 이미 있는 노드의 LaTeX를 고친다. 빈 값으로 확인하면 노드를 지운다. */
async function promptEdit(editor, getPos, displayMode, currentLatex) {
    ...
    editor
        ...
        if (!target || target.type.name !== name)
            return false;
...
function buildNodeView(displayMode) {
    return ({ editor, node, getPos, HTMLAttributes, }) => {
        ...
        return {
            dom,
            update: (updated) => {
                if (updated.type.name !== current.type.name)
                    return false;
                current = updated;
                paint();
                return true;
...
function pushUniqueTarget(targets, from, to, text) {
...
function collectMathTargets(state, mathInline) {
    const targets = [];
    ...
    if (selection.$from.parent.type === mathInline) {
        ...
        pushUniqueTarget(targets, selection.$from.before(depth), selection.$from.after(depth), selection.$from.parent.textContent);
    }
...
export const MathInline = Node.create({
    ...
    addStorage() {
        return {
            promptMath: this.options.promptMath,
        };
    },
    parseHTML() {
    ...
    addCommands() {
        return {
            ...
            promptMathInline: () => ({ editor }) => {
                void promptInsert(editor, false);
                return true;
            },
            toggleMathInline: () => ({ state, tr, dispatch }) => {
