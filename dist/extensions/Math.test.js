...
import { MathInline, MathDisplay } from './Math';
describe('Math extension', () => {
    let editor;
    function createEditor(content = '<p></p>', promptMath = null) {
        editor = new Editor({
    ...
    describe('저장 포맷 round-trip', () => {
        const SAVED = '<p>앞 <math-inline class="math-node">x^2</math-inline> 뒤</p>' +
    ...
    describe('붙여넣기 변환 (transformPasted)', () => {
        ...
        it('$$ 만 있는 줄로 감싼 여러 줄도 한 덩어리 수식 블록이 된다', () => {
            createEditor();
            const out = pasteTransform(editor)(sliceOf(paragraph('$$'), paragraph('a + b'), paragraph('$$')));
